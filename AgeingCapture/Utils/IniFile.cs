...
namespace AutoCali.Infrastructure
{
    /// <summary>
    /// ini文件类
    /// </summary>
    public class IniFile
    {
        ...
        /// <returns></returns>
        public string IniReadValue(string section, string name, string defaultVal = "")
        {
            StringBuilder strSb = new StringBuilder(256);
            GetPrivateProfileString(section, name, defaultVal, strSb, 256, this.m_FileName);
        ...
        public List<string> ReadSections()
        {
            ...
            uint len = GetPrivateProfileStringA(null, null, null, buf, buf.Length, m_FileName);
            int j = 0;
            for (int i = 0; i < len; i++)
                if (buf[i] == 0)
                {
        ...
        public List<string> ReadKeys(string SectionName)
        {
            List<string> result = new List<string>();
            Byte[] buf = new Byte[65536];
            uint len = GetPrivateProfileStringA(SectionName, null, null, buf, buf.Length, m_FileName);
            int j = 0;
            for (int i = 0; i < len; i++)
                if (buf[i] == 0)
                {
